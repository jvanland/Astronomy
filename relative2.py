...
filename = "inspiral_all.dat"
f1 = open(filename, 'r')
data = array([map(float, line.split()) for line in f1])
subset1 = data[data[:,0] == 5]
subset2 = data[data[:,0] == 2]
subset3 = data[data[:,0] == 1]
mbh = array([bit[0] for bit in data]) 
bnum = array([bit[1] for bit in data]) 
vnum = array([bit[2] for bit in data])
tfin5 = array([bit[4] for bit in subset1])
tfin2 = array([bit[4] for bit in subset2])
tfin1 = array([bit[4] for bit in subset3])
f1.close
...
p1 = plot(sorted(tfin5/56000),cum_ins5,'m')
p1 = plot(sorted(tfin1/20000),cum_ins1,'b--')
p3 = plot(sorted(tfin2/30000),cum_ins2,'g')
